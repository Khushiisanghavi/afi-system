...
export default function WellbeingProfilePage() {
  ...
  return (
    <div style={{ position: "relative", zIndex: 1, minHeight: "100vh" }}>
      ...
      <motion.div variants={STAGGER_CONTAINER} initial="hidden" animate="show" className="container-section" style={{ display: "flex", flexDirection: "column", gap: "1.75rem", position: "relative", zIndex: 1 }}>
        ...
        <motion.div variants={FADE_UP}>
          ...
          <p className="sans" style={{ color: "rgba(232,232,240,0.5)", fontSize: "0.95rem", maxWidth: "36rem" }}>
        ...
        {profile && tier && (
          <>
            {/* Tier + stats */}
            <motion.div variants={STAGGER_CONTAINER} initial="hidden" animate="show" style={{ display: "flex", flexDirection: "column", gap: "1.75rem" }}>
              <motion.div variants={FADE_UP} className="card-glass" style={{ padding: "2.5rem", borderColor: tier.border, background: tier.bg }}>
                <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1rem", flexWrap: "wrap" }}>
                  <span className="display-font" style={{ fontSize: "1.75rem", fontWeight: 600, color: "#ffffff" }}>
                    Profile: {profile.profile_tier}
                  </span>
                  <span className="mono" style={{ padding: "0.4rem 1rem", borderRadius: "100px", fontSize: "0.9rem", fontWeight: 700, color: tier.color, background: `${tier.color}18`, border: `1px solid ${tier.color}44`, letterSpacing: "0.05em" }}>
                    {profile.attention_fragmentation_index.toFixed(0)} / 100 AFI
                  </span>
                </div>
                <p className="sans" style={{ color: "rgba(232,232,240,0.6)", fontSize: "0.95rem", lineHeight: 1.65, marginBottom: "2rem", maxWidth: "48rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "1rem" }}>
                  {[
                    ...
                    <div key={s.label} style={{ background: "rgba(255,255,255,0.03)", borderRadius: "16px", padding: "1.5rem", textAlign: "center", border: "1px solid rgba(255,255,255,0.05)" }}>
                      <div className="sans" style={{ fontSize: "0.75rem", color: "rgba(232,232,240,0.45)", marginBottom: "0.5rem", textTransform: "uppercase", letterSpacing: "0.05em" }}>{s.label}</div>
                      <div className="mono" style={{ fontSize: "1.75rem", fontWeight: 700, color: "#ffffff" }}>{s.val}</div>
                    </div>
                  ))}
                </div>
              </motion.div>
              ...
              {profile.insights?.length > 0 && (
                <motion.div variants={FADE_UP} className="card-glass" style={{ padding: "2.5rem" }}>
                  <div className="sans" style={{ fontSize: "0.85rem", color: "rgba(232,232,240,0.4)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "1.5rem" }}>Insights</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                    {profile.insights.map((ins, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "1rem" }}>
                        <span style={{ width: "1.75rem", height: "1.75rem", borderRadius: "50%", flexShrink: 0, background: "rgba(45,212,191,0.12)", border: "1px solid rgba(45,212,191,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.85rem", fontWeight: 700, color: "#2dd4bf", fontFamily: "monospace" }}>{i + 1}</span>
                        <p className="sans" style={{ fontSize: "0.95rem", color: "rgba(232,232,240,0.7)", lineHeight: 1.6 }}>{ins}</p>
                      </div>
                    ))}
                  </div>
                </motion.div>
              ...
              <motion.div variants={FADE_UP} className="card-glass" style={{ padding: "2.5rem", borderColor: "rgba(45,212,191,0.2)", background: "rgba(45,212,191,0.04)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.25rem" }}>
                  ...
                  <span className="sans" style={{ fontWeight: 600, fontSize: "1.1rem", color: "#2dd4bf" }}>AI Attention Analysis</span>
                ...
                {llmInsight ? (
                  ...
                  <p className="sans" style={{ fontSize: "0.9rem", color: "rgba(232,232,240,0.45)", fontStyle: "italic" }}>Analysing your attention profile…</p>
                ) : (
                  <p className="sans" style={{ fontSize: "0.9rem", color: "rgba(232,232,240,0.45)" }}>
              ...
              {sessions.length > 0 && (
                <motion.div variants={FADE_UP} className="card-glass" style={{ padding: "2.5rem" }}>
                  <div className="sans" style={{ fontSize: "0.85rem", color: "rgba(232,232,240,0.4)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "1.5rem" }}>Recent Content Breakdown</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "1rem", maxHeight: "20rem", overflowY: "auto", paddingRight: "1rem" }}>
                    {sessions.slice(0, 20).map((s, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: "1rem", fontSize: "0.9rem" }}>
                          <span style={{ width: "10px", height: "10px", borderRadius: "50%", background: info.dot, flexShrink: 0 }} />
                          ...
                          <span className="mono" style={{ fontSize: "0.9rem", color: "rgba(232,232,240,0.4)" }}>{s.final_afi.toFixed(0)}</span>
                          <span className="sans" style={{ fontSize: "0.85rem", fontWeight: 600, color: info.color, minWidth: "5rem", textAlign: "right" }}>{s.harm_tier}</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              <motion.div variants={FADE_UP}>
                <div className="sans" style={{ fontSize: "0.95rem", fontWeight: 600, color: "rgba(232,232,240,0.5)", marginBottom: "1.25rem" }}>
                  What each tier means for your attention
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "1rem" }}>
                  {Object.entries(HARM_INFO).map(([t, info]) => (
                    <div key={t} style={{ borderRadius: "16px", padding: "1.75rem", background: info.bg, border: `1px solid ${info.border}` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.75rem" }}>
                        <span style={{ width: "10px", height: "10px", borderRadius: "50%", background: info.dot }} />
                        <span className="sans" style={{ fontSize: "0.95rem", fontWeight: 600, color: info.color }}>{t}</span>
                      </div>
                      <p className="sans" style={{ fontSize: "0.85rem", color: "rgba(232,232,240,0.6)", lineHeight: 1.6 }}>{info.desc}</p>
                    </div>
                  ))}
                </div>
              </motion.div>
              ...
              {checkins.length > 0 && (
                <motion.div variants={FADE_UP} className="card-glass" style={{ padding: "2.5rem" }}>
                  <div className="sans" style={{ fontSize: "0.85rem", color: "rgba(232,232,240,0.4)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "1.5rem" }}>Focus Quality Log</div>
                    ...
                    {checkins.slice(0, 7).reverse().map((c, i) => {
                      ...
                      return (
                        <div key={i} style={{ textAlign: "center" }}>
                          <div style={{ aspectRatio: "1", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", background: `${scoreColor}15`, border: `1px solid ${scoreColor}40`, fontFamily: "monospace", fontSize: "1rem", fontWeight: 700, color: scoreColor }}>
                            {c.focus_quality}/5
                          </div>
                          <div className="sans" style={{ fontSize: "0.75rem", color: "rgba(232,232,240,0.4)", marginTop: "0.5rem" }}>
              ...
              {/* CTA */}
              <motion.div variants={FADE_UP} style={{ display: "flex", justifySelf: "center", placeContent: "center", paddingBottom: "2rem" }}>
                <Link href="/wellbeing/plan" style={{ display: "inline-block", background: "linear-gradient(135deg, #2dd4bf, #14b8a6)", color: "#05050f", textDecoration: "none", padding: "0.8rem 2rem", borderRadius: "100px", fontFamily: "'Inter', sans-serif", fontSize: "0.95rem", fontWeight: 700, boxShadow: "0 4px 16px rgba(45,212,191,0.25)" }}>
