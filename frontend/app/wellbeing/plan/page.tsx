...
export default function WellbeingPlanPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([p, prof]) => {
        setPlan(p); setTier(prof.profile_tier || "");
      ...
      <div className="container-section" style={{ display: "flex", flexDirection: "column", gap: "1.5rem", maxWidth: "52rem", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
          ...
          <button onClick={handleRefresh} disabled={refreshing} style={{
            ...
            color: "rgba(232,232,240,0.55)", background: "none",
            border: "1px solid rgba(167,139,250,0.2)", cursor: "pointer", transition: "all 0.2s",
                ...
                {plan.daily_goals.map((goal, i) => (
                  <div key={i} onClick={() => setChecked((prev) => { const n = [...prev]; n[i] = !n[i]; return n; })}
                    style={{
                      ...
                      background: checked[i] ? "rgba(45,212,191,0.06)" : "rgba(255,255,255,0.03)",
                      border: `1px solid ${checked[i] ? "rgba(45,212,191,0.3)" : "rgba(255,255,255,0.08)"}`,
                      transition: "all 0.2s ease",
                      display: "flex", alignItems: "flex-start", gap: "1rem",
                    }}>
                    <div style={{
                      width: "1.4rem", height: "1.4rem", borderRadius: "50%", flexShrink: 0, marginTop: "0.1rem",
                      border: `2px solid ${checked[i] ? "#2dd4bf" : "rgba(255,255,255,0.2)"}`,
                      background: checked[i] ? "#2dd4bf" : "transparent",
                      display: "flex", alignItems: "center", justifyContent: "center",
                      transition: "all 0.2s ease", fontSize: "0.7rem", color: "#05050f", fontWeight: 700,
                    }}>
                      {checked[i] && "✓"}
                    </div>
                    <div>
                      <p className="sans" style={{
                        fontSize: "0.9rem", fontWeight: 500, lineHeight: 1.4, marginBottom: "0.3rem",
                        color: checked[i] ? "rgba(232,232,240,0.35)" : "#ffffff",
            ...
            <div className="card-glass" style={{ padding: "1.75rem" }}>
              ...
              {checkinSent ? (
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem" }}>
                    {[1, 2, 3, 4, 5].map((n) => (
                      <button key={n} onClick={() => setCheckinScore(n)} style={{
                        flex: 1, padding: "0.75rem", borderRadius: "10px", fontSize: "0.9rem", fontWeight: 700,
                        fontFamily: "monospace", cursor: "pointer", transition: "all 0.2s",
                        background: checkinScore === n ? "#2dd4bf" : "rgba(255,255,255,0.04)",
                        color: checkinScore === n ? "#05050f" : "rgba(232,232,240,0.6)",
                        border: `1.5px solid ${checkinScore === n ? "#2dd4bf" : "rgba(255,255,255,0.1)"}`,
                      }}>{n}</button>
                    ))}
                  </div>
                  <textarea
                    ...
                    rows={2}
                    style={{
                      width: "100%", fontSize: "0.875rem", padding: "0.75rem 1rem",
                  ...
                  <button onClick={handleCheckin} disabled={checkinScore === null} style={{
                    width: "100%", padding: "0.75rem",
                    background: checkinScore !== null ? "linear-gradient(135deg, #2dd4bf, #14b8a6)" : "rgba(255,255,255,0.06)",
                    color: checkinScore !== null ? "#05050f" : "rgba(232,232,240,0.3)",
                    border: "none", borderRadius: "10px", fontSize: "0.875rem", fontWeight: 700,
                    cursor: checkinScore !== null ? "pointer" : "not-allowed",
